...
export const generate = ({ describe, it, expect, State }) => {

const _checkHeaders = ({ res, item }) => {
...
const _checkListHeaders = ({ entry, item }) => {
	if (!State.spec_version)
		throw new Error('State.spec_version not set');
...
describe('OPTIONS', () => {
	...
	['GET', 'PUT', 'DELETE'].forEach(method => {
		...
		(typeof window === 'undefined' ? it : it.skip)(`${ typeof window !== 'undefined' ? '[skip in browser] ' : '' }` + `handles ${ method }`, async () => {
			const origin = typeof location !== 'undefined' ? location.origin : (State.origin || stub.origin());
...
describe('other user', () => {
	...
	['HEAD', 'GET', 'PUT', 'DELETE'].forEach(method => {
		...
		it(`rejects ${ method }`, async () => {
			const pattern = new RegExp(`\\/${ State.account_handle }$`);
...
describe('create', () => {
	...
	describe.skip('target file path is an existing folder', () => {
	...
	describe.skip('folder in path is existing file', () => {
	...
	describe('binary file', () => {
		...
		it('returns 200', async () => {
			const put = await State.storage.put(`${ stub.tid() }.jpg`, await readFile(join(__dirname, 'image.jpg')), {
...
describe('read', () => {
	...
	describe('binary file', () => {
		...
		it('returns 200', async () => {
			...
			const data = await readFile(join(__dirname, 'image.jpg'));
